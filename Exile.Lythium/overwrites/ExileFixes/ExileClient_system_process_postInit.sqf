...
else 
{
	if (hasInterface) then
	{
		...
		if (!isServer) then 
		{
			...
			[1, ExileClient_object_player_safezone_checkSafezone, [], true] call ExileClient_system_thread_addtask;
			[10, ExileClient_object_player_save, [], true] call ExileClient_system_thread_addtask;
